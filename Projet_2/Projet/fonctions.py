...
def calculer_score(joueur):
    """
    Calcule le score selon les règles officielles :
    - 1 point par carré posé.
    - Bonus +15 si toutes les pièces sont posées.
    - Bonus +20 si la dernière pièce posée était le monomino (1x1).
    Note : Ici on calcule les points POSITIFS (ce qui a été posé).
    """
    score = 0
    # On définit ce qu'est la main complète au départ (21 pièces, 89 carrés au total)
    # Dans votre code, vous avez 21 pièces.
    
    # Si le joueur n'a plus de pièces
    if not joueur.pieces:
        score = 89  # Total des carrés pour 21 pièces
        if joueur.derniere_piece_etait_un_par_un:
            score += 20
        else:
            score += 15
    else:
        # On calcule les points des pièces qu'il N'A PLUS (celles posées)
        # Total théorique (89) - Somme des carrés restants
        carres_restants = sum(len(p) for p in joueur.pieces)
        score = 89 - carres_restants
